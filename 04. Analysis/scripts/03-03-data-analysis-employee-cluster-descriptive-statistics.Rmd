...
params:
  ...
  data_file_1: "clusters-employee-2023-06-01.rdata"
...
d <-
  assignments %>%
  select(k, person_id, .cluster, specification) %>%
  full_join(prep_employees) %>% 
  mutate(
    .cluster = fct_na_value_to_level(.cluster, level = "(Missing)")
  )
```
Review data.
...
prep_a <- bind_rows(
  ...
  d %>%
    summaries(.cluster) %>%
    mutate(group = glue::glue("Cluster: {.cluster}")) %>%
    select(-.cluster),
  d %>% 
...
```{r}
gg_density <- function(in_data,in_variable)
{
  ...
  in_data %>% 
  ggplot(aes(x = !!in_variable_sym, color = .cluster, fill = .cluster)) +
  geom_density(alpha = .25) +
    geomtextpath::geom_textdensity(aes(label = .cluster), 
                   hjust = "ymax", 
                   vjust = 1, text_only = TRUE, text_smoothing = 0, straight = TRUE) + 
    oescolorrrs::scale_color_oes(palette = 'accent colors') +
    oescolorrrs::scale_fill_oes(palette = 'accent colors') +
    theme_fivethirtyeight() +
    labs(
      title = glue::glue("Histogram of `{in_variable}`"),
      caption = glue::glue("For specification {spec} with {clus} clusters.")
    ) +
    theme(
