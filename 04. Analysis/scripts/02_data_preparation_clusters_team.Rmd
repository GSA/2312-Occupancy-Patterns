...
Join working days, working days per status, and team members into one datafram by `supervisor_e_mail_address`.
...
Identify employees who telework and collect `person_id`, `dates`, `location`, and `supervisor_e_mail_address`.
...
prep_c <- 
  prep_b %>% 
  group_by(person_id,date,supervisor_e_mail_address) %>% 
  summarise(telework_team_members = n_distinct(person_id_joined)) %>% 
  ungroup()
```
