...
cohort_panel %>% 
  ...
  summarise(cor = cor(Fri,Wed,method = "kendall", use = "complete.obs"))
```
...
cohort_panel %>% 
  ...
  group_by(person_id) %>%
  # filter(person_id == "10070")
  summarise(cor = cor(as.numeric(Fri),as.numeric(Wed),method = "spearman", use = "complete.obs"))
# %>% 
  # count(daily_status_imputed)
```
...
```{r}
cor(check_in_panel$daily_status_imputed,check_in_panel$date_day_of_week)
```
