...
-   Telework has become very common

-   Much of GSA staff telework

-   How employees use telework is not clear

-   Defining and describing different work patterns may help to understand organizational health

-   By understanding how staff work (remote, hybrid, or onsite), different strategies may be developed to improve productivity and staff engagement

# How did the evaluation work?

-   Data:

    -   GSA check-in data, GSA staff roster

    -   March - September 2023

-   All GSA staff

    -   Categorized work patterns by remote or onsite

    -   Rules: Mostly remote or onsite, staff category (flexible, onsite, etc), flexible onsite

    -   Clusters: K-means clusters defined using frequency of work location status, collocation with team members, and team remote work
...
# Clusters: How much telework? (proportion of working) (rain)
...
# Telecommuters, office visitors, and regions
...
-   Telework is very common - most GSA employees telework

-   Telecommuters and office visitors are very common for those that live in states neighboring DC

-   Employees living in DC are more likely to come to the office 

-   PBS has considerable more onsite work than other offices


# Appendix

# Position Categories: How much telework?
...
p_2 <- 
  ...
  mutate(position_category_description = fct_lump_min(position_category_description, min = prep_v_01),
         position_category_description = fct_infreq(position_category_description)) %>%
  ggplot(
    aes(x = daily_status_teleworking, color = position_category_description, fill = position_category_description)
  ) +
  geom_density(alpha = .5) +
  # geom_vline(data = prep_01, aes(xintercept = num, color = position_category_description)) +
  geom_text(data=prep_02, aes(x=x, y=y, label=position_category_description), size = 3, hjust = "inward") +
  xlab("Days of telework") +
  ylab("Density") +
  theme_light() +
  theme(legend.position = "None")
...
# Position Categories: How much onsite work?
...
p_2 <- 
  ...
  mutate(position_category_description = fct_lump_min(position_category_description, min = prep_v_01),
         position_category_description = fct_infreq(position_category_description)) %>%
  ggplot(
    aes(x = daily_status_reporting_to_job_site, color = position_category_description, fill = position_category_description)
  ) +
  geom_density(alpha = .5) +
  # geom_vline(data = prep_01, aes(xintercept = num, color = position_category_description)) +
  geom_text(data=prep_02, aes(x=x, y=y, label=position_category_description), size = 3, hjust = "inward") +
...
# Rule-based groups: How much telework?
...
prep_d_01 <-
  ...
  select(person_id, 
         ...
         rule_based_all_remote,
         rule_based_telework_onsite_minimum,
         daily_status_teleworking) %>% 
  pivot_longer(starts_with("rule_based_")) %>% 
  filter(value == 1) %>% 
  mutate(
    name = str_remove(name, "rule_based_"),
    name = str_remove(name, "_working"),
...
p_2 <- 
  prep_d_01 %>% 
  ggplot(
    aes(x = daily_status_teleworking, color = name, fill = name)
  ) +
  geom_density(alpha = .5) +
  # geom_vline(data = prep_01, aes(xintercept = num, color = name)) +
  geom_text(data=prep_02, aes(x=x, y=y, label=name), size = 3, hjust = "inward") +
  xlab("Days of telework") +
  ylab("Density") +
  theme_light() +
  theme(legend.position = "None")
...
p_2 <- 
  prep_d_01 %>% 
  ggplot(
    aes(x = daily_status_reporting_to_job_site, color = name, fill = name)
  ) +
  geom_density(alpha = .5) +
  # geom_vline(data = prep_01, aes(xintercept = num, color = name)) +
  geom_text(data=prep_02, aes(x=x, y=y, label=name), size = 3, hjust = "inward") +
...
# Onsite Flexible: Rule-based groups
